...
function initFirebase() {
  if (admin.getApps().length === 0) {
    ...
    if (projectId) {
      ...
      console.log(
        "Firebase Admin initialized lazily with projectId:",
        projectId,
      );
    } else {
      throw new Error(
        "FIREBASE_PROJECT_ID is not set in environment or config.",
      );
    }
  }
}
...
const apiLimiter = rateLimit({
  ...
  message: {
    error:
      "Terlalu banyak permintaan (Rate limit exceeded). Silakan coba lagi nanti.",
  },
...
apiRoutes.post("/xendit/checkout", async (req, res) => {
  try {
    ...
    const { amount, plan, email, description, planId, addMonths, promoId } =
      req.body;
    ...
    const origin =
      req.headers.origin ||
      "https://ais-dev-olxozwjiavip4zo5jfxg4e-864064225498.asia-southeast1.run.app";
    ...
    return res
      .status(400)
      .json({ error: errorDetail || "Failed to create checkout link" });
  }
});
      ...
      if (userDoc.exists) {
        ...
        if (promoId && promoId !== "none") {
          ...
          try {
            ...
            console.log(
              `Berhasil menaikkan usageCount untuk promo: ${promoId}`,
            );
          } catch (promoErr) {
            console.error(
              `Gagal update usageCount promo ${promoId}:`,
              promoErr,
            );
          }
        }
        ...
        console.log(
          `Berhasil update plan untuk user ${uid} ke ${plan} selama +${addMonths} bulan`,
        );
  ...
  try {
    ...
    for (const ws of wsSnap.docs) {
      ...
      if (process.env.INSTAGRAM_MANUAL_TOKEN) {
        ...
        try {
          const profileRes = await fetch(
            `https://graph.instagram.com/v19.0/me?fields=id,username&access_token=${process.env.INSTAGRAM_MANUAL_TOKEN}`,
          );
          if (profileRes.ok) {
...
apiRoutes.post("/meta/sync-secrets", async (req, res) => {
  ...
  try {
    ...
    if (process.env.META_MANUAL_TOKEN) {
      ...
      try {
        const profileRes = await fetch(
          `https://graph.facebook.com/v19.0/me?fields=id,name&access_token=${process.env.META_MANUAL_TOKEN}`,
        );
        if (profileRes.ok) {
          const data = await profileRes.json();
          accountId = data.id || accountId;
...
apiRoutes.get("/meta/auth", async (req, res) => {
  ...
  const envToken =
    platform === "instagram"
      ? process.env.INSTAGRAM_MANUAL_TOKEN
      : process.env.META_MANUAL_TOKEN;
  ...
  if (envToken) {
    try {
      ...
      let accountName =
        platform === "instagram" ? "Manual IG Account" : "Manual FB Account";
      ...
      try {
        ...
        console.warn(
          "Could not fetch profile info for manual token, using fallback names.",
        );
      }
  ...
  console.log(
    "Initiating OAuth with Redirect URI:",
    redirectUri,
    "for platform:",
    platform,
  );
  ...
  if (platform === "instagram") {
    if (!INSTAGRAM_APP_ID) {
      return res
        .status(500)
        .send(
          "INSTAGRAM_APP_ID is not configured on the server. Please add it in AI Studio Secrets.",
        );
    }
    const scope =
    ...
    if (!META_APP_ID) {
      return res
        .status(500)
        .send("META_APP_ID is not configured on the server.");
    }
    const scope =
...
apiRoutes.get("/meta/data", async (req, res) => {
  const { workspaceId, platform, type, clientAccessToken, clientAccountId } =
    req.query;

  if (!workspaceId || !platform) {
    return res
      .status(400)
      .json({ error: "workspaceId and platform are required" });
  }
...
apiRoutes.post("/meta/reply-comment", async (req, res) => {
  ...
  if (!workspaceId || !platform || !commentId || !message) {
    return res.status(400).json({
      error:
        "Missing required fields (workspaceId, platform, commentId, message)",
    });
  }
    ...
    if (accessToken && !accessToken.startsWith("mock_")) {
      ...
      if (data.error) {
        ...
        return res.json({
          success: true,
          simulated: true,
          note: "Meta API returned error, simulated instead",
          error: data.error,
        });
      }
      ...
      console.log(
        `Simulated Meta comment reply to ${commentId} (No valid Meta connection)`,
      );
      return res.json({
        success: true,
        simulated: true,
        message: "Comment reply simulated successfully",
      });
    }
  } catch (err: any) {
...
apiRoutes.post("/meta/send-message", async (req, res) => {
  ...
  if (!workspaceId || !platform || !recipientId || !message) {
    return res.status(400).json({
      error:
        "Missing required fields (workspaceId, platform, recipientId, message)",
    });
  }
  ...
  try {
    ...
    if (accessToken && !accessToken.startsWith("mock_")) {
      ...
      console.log(
        `Simulated Meta DM message to ${recipientId} (No valid Meta connection)`,
      );
      return res.json({
        success: true,
        simulated: true,
...
apiRoutes.post("/meta/publish-post", async (req, res) => {
  ...
  try {
    ...
    if (accessToken && !accessToken.startsWith("mock_")) {
      console.log(
        `[API] Publishing real post to ${platform} for workspace ${workspaceId}`,
      );
      ...
      return res.json({
        success: true,
        simulated: true,
        note: "Meta API publish mock successful",
      });
    } else {
      console.log(
        `[API] Simulated publishing to ${platform} (No valid connection)`,
      );
      return res.json({
        success: true,
        simulated: true,
...
apiRoutes.post("/gemini", apiLimiter, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        return res.status(500).json({
          error: "Sistem belum dikonfigurasi sepenuhnya. " + initErr.message,
        });
      }
    ...
    apiKeyName = process.env.VITE_GEMINI_API_KEY
      ? "VITE_GEMINI_API_KEY"
      : "GEMINI_API_KEY";

    console.log(
      `[API KEY INFO] Menggunakan kunci: ${apiKeyName} (Prefix: ${apiKey ? apiKey.substring(0, 6) : "none"}). Jika Anda menggunakan kunci gratis, batas limit berlaku.`,
    );
    if (!apiKey) {
      return res.status(500).json({
        error:
          "GEMINI_API_KEY belum dikonfigurasi di server (Settings > Secrets).",
      });
    }
        ...
        functionDeclarations: [
          {
            ...
            description:
              "Mengambil data analitik, metrik, performa konten, dan jadwal posting dari workspace media sosial pengguna. Gunakan tool ini jika pengguna menanyakan data performa, jadwal, konten, atau metrik (views, reach, ER, likes).",
            parameters: {
              type: "OBJECT",
              properties: {
                dataType: {
                  type: "STRING",
                  description:
                    "Jenis data yang ingin diambil: 'content_performance' (untuk melihat performa konten individu), 'analytics_summary' (untuk melihat total keseluruhan metrik termasuk breakdown per platform), atau 'scheduled_posts' (untuk melihat konten yang akan dipublikasikan).",
                },
                month: {
                  type: "STRING",
                  description:
                    "Filter bulan dalam format YYYY-MM (contoh: '2025-01' untuk Januari 2025). Kosongkan untuk mengambil semua data terbaru.",
                },
              },
              required: ["dataType"],
            },
    ...
    while (retries > 0) {
      try {
        ...
        while (
          ...
          console.log(
            `[Gemini] Model memanggil tool: ${call.name} dengan args:`,
            call.args,
          );
            ...
            if (call.name === "fetch_workspace_data" && workspaceId) {
              ...
              if (month) {
                ...
                docs = docs.filter((d) => {
                  if (d.year && d.month) {
                    return (
                      String(d.year) === targetYear &&
                      String(d.month).padStart(2, "0") === targetMonth
                    );
                  }
                  if (d.date) {
                    ...
                    if (!isNaN(dateObj.getTime())) {
                      return (
                        String(dateObj.getFullYear()) === targetYear &&
                        String(dateObj.getMonth() + 1).padStart(2, "0") ===
                          targetMonth
                      );
                    }
                    return (
                      String(d.date).includes(targetYear) &&
                      (String(d.date).includes(targetMonth) ||
                        String(d.date).includes(String(Number(targetMonth))))
                    );
                  }
                  return false;
                });
              ...
              if (dataType === "analytics_summary") {
                ...
                docs.forEach((c) => {
                  ...
                  const rep =
                    (metrics.reposts || 0) + (adsMetrics.reposts || 0);
                  ...
                  const plats = Array.isArray(c.platform)
                    ? c.platform
                    : typeof c.platform === "string"
                      ? c.platform.split(",").map((s: string) => s.trim())
                      : ["unknown"];
                ...
                Object.keys(perPlatform).forEach((p) => {
                  const eng =
                    perPlatform[p].likes +
                    perPlatform[p].comments +
                    perPlatform[p].shares +
                    perPlatform[p].saves +
                    perPlatform[p].reposts;
                  perPlatform[p].engagementRate =
                    perPlatform[p].reach > 0
                      ? ((eng / perPlatform[p].reach) * 100).toFixed(2) + "%"
                      : "0%";
                });
              ...
              result = {
                error: "Tool tidak didukung atau workspaceId tidak diberikan.",
              };
            }
          } catch (err: any) {
    ...
    return res.json({
      text:
        response?.text ||
        "Maaf, aku sedang kesulitan memproses data tersebut dari database. Bisa kasih tahu lebih spesifik lagi?",
    ...
    if (isQuotaError) {
      ...
      if (isLimitZero) {
        return res.status(429).json({
          error: `Akses ditolak oleh Google (limit: 0). Server saat ini menggunakan kunci yang berakhiran: "...${keySuffix}" (Nama Secret: ${apiKeyName}). Jika ini BUKAN akhiran dari API Key baru Anda (misalnya Anda mengharapkan berakhiran NO2g), berarti Anda perlu merestart server atau format nama secret salah. Pastikan nama secret adalah "VITE_GEMINI_API_KEY".`,
        });
      }
      return res.status(429).json({
        error: `Maaf, kuota API Gemini telah habis (Quota Exceeded). Key digunakan: ${apiKeyName} (Akhiran: ...${keySuffix}). Jika Anda menggunakan API Key gratis, batas limit Google gratis berlaku. (Pesan Asli: ${error?.message})`,
      });
    }
    ...
    if (isBillingError) {
      ...
      return res.status(403).json({
        error: `Akses ditolak (Forbidden). Server menggunakan API Key berakhiran: "...${keySuffix}" (${apiKeyName}). Jika ini BUKAN akhiran API Key baru, mohon cek kembali menu Secrets, pastikan namanya VITE_GEMINI_API_KEY, lalu restart server.`,
      });
    }

    return res
      .status(500)
      .json({ error: error?.message || "Gagal mendapatkan respon dari AI." });
  }
});
...
app.use((req, res, next) => {
  if (req.url.startsWith("/api") || process.env.VERCEL) {
    res.status(404).json({
      error: `API Route not found in Express: ${req.method} ${req.originalUrl || req.url}`,
    });
  } else {
    next();
  }
    ...
    cron.schedule("*/15 * * * *", async () => {
      try {
        ...
        for (const wsDoc of workspacesSnap.docs) {
          const contentSnap = await wsDoc.ref
            .collection("content")
          ...
          for (const contentDoc of contentSnap.docs) {
            console.log(
              `[Cron] Publishing scheduled post ${contentDoc.id} for workspace ${wsDoc.id}`,
            );
