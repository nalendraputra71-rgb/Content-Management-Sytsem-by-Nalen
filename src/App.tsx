...
import { 
  ...
  handleFirestoreError, testFirestoreConnection, where, getDocs, limit, checkAndNotifyQuotaError
} from "./firebase";
...
import { Calendar, Download, X, CheckCircle2, AlertTriangle, Trash2, Sparkles, RefreshCw, Clock } from "lucide-react";
...
function MaintenanceCountdown({ lang }: { lang: "id" | "en" }) {
  ...
  useEffect(() => {
    const updateCountdown = () => {
      ...
      // target 14:00 WIB, which is 07:00 UTC
      target.setUTCHours(7, 0, 0, 0);

      // if now is past 14:00 WIB today, target tomorrow's 14:00 WIB
  ...
  return (
    <div className="flex flex-col items-start">
      <div className="flex items-center gap-1.5 mb-4 text-[#3B82F6]">
        <Clock size={14} className="stroke-[2.5]" />
        <span className="text-xs font-bold uppercase tracking-wider">
          {lang === "id" ? "Estimasi Kembali" : "System Resuming In"}
        </span>
      </div>

      {/* Big highlighted countdown blocks */}
      <div className="flex items-center justify-start gap-3 md:gap-4">
        <div className="flex flex-col items-center">
          <div className="w-16 h-16 md:w-20 md:h-20 bg-blue-50/80 text-[#3B82F6] font-mono text-3xl md:text-4xl font-black rounded-2xl flex items-center justify-center border border-blue-100/50 shadow-xs">
            {timeLeft.hours}
          </div>
          <span className="text-[10px] md:text-xs font-bold text-stone-400 mt-2.5 uppercase tracking-wider">
            {lang === "id" ? "Jam" : "Hours"}
          </span>
        </div>
        
        <div className="text-xl md:text-2xl font-black text-blue-200/80 mb-6">:</div>
        ...
        <div className="flex flex-col items-center">
          <div className="w-16 h-16 md:w-20 md:h-20 bg-[#3B82F6] text-white font-mono text-3xl md:text-4xl font-black rounded-2xl flex items-center justify-center border border-blue-600/10 shadow-[0_8px_20px_rgba(59,130,246,0.12)]">
            {timeLeft.seconds}
          </div>
          <span className="text-[10px] md:text-xs font-bold text-stone-400 mt-2.5 uppercase tracking-wider">
            {lang === "id" ? "Detik" : "Secs"}
...
export default function App() {
      ...
      if (profile?.plan && profile.plan !== "free" && profile.plan !== "trial") {
        try {
          ...
          if (snap.exists()) {
            ...
            if (matched) setPlanDetails(matched.data());
         ...
         try {
           ...
           if (snap.exists()) {
             ...
             setPlanDetails({
               ...
               maxAiGenerations: freeData.limits?.aiGenerationPerMonth ?? 10,
  ...
  if (quotaExceeded) {
    return (
      <div className="min-h-screen w-full flex flex-col md:flex-row items-center justify-center p-8 md:p-16 relative overflow-hidden" style={{ backgroundColor: "#FAF9F6", fontFamily: "Plus Jakarta Sans, system-ui, sans-serif" }}>
        
        {/* Soft Background Blobs matching Hubify brand (Blue & Cream) */}
        <motion.div 
          animate={{ scale: [1, 1.12, 1], rotate: [0, 40, 0] }} 
          transition={{ duration: 15, repeat: Infinity, ease: "easeInOut" }} 
          className="absolute -top-24 -left-24 w-96 h-96 bg-blue-100/40 rounded-full blur-3xl pointer-events-none" 
        />
        
        <motion.div 
          animate={{ scale: [1, 1.08, 1], y: [0, 15, 0] }} 
          transition={{ duration: 18, repeat: Infinity, ease: "easeInOut", delay: 2.5 }} 
          className="absolute -bottom-24 -right-12 w-[500px] h-[500px] bg-amber-100/30 rounded-full blur-3xl pointer-events-none" 
        />

        {/* Outer Split Wrapper */}
        <div className="w-full max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between relative z-10 gap-12 md:gap-16">
          
          {/* Left Side: Creative & Organic Huby Visual */}
          <div className="w-full md:w-1/2 flex items-center justify-center relative min-h-[300px] md:min-h-[480px]">
            {/* Wobbly Organic Backdrop Circle */}
            <motion.div 
              animate={{ 
                borderRadius: [
              ...
              className="absolute w-72 h-72 md:w-[440px] md:h-[440px] bg-gradient-to-tr from-blue-100/40 to-amber-50/30 backdrop-blur-md shadow-sm border border-blue-50/30"
            />

            {/* Huby Character floating very gently */}
            <motion.div
              animate={{ 
                y: [0, -6, 0]
              }}
            ...
            >
              <img 
                ...
                alt="Huby is Sad" 
                className="w-full h-full object-contain filter drop-shadow-[0_12px_28px_rgba(44,32,22,0.06)]"
                referrerPolicy="no-referrer"
              />
            </motion.div>
          </div>

          {/* Right Side: Descriptive Left-Aligned Words */}
          <div className="w-full md:w-1/2 flex flex-col justify-center items-start text-left max-w-xl">
            
            {/* Asymmetrical Artistic Headline */}
            <h1 className="text-3.5xl md:text-5xl font-black text-[#2C2016] tracking-tight leading-tight mb-6">
              {lang === "id" ? (
                <>Aduh, Huby<br/><span className="text-blue-600 font-serif italic font-normal">sedang sedih...</span></>
              ) : (
                <>Oh no, Huby is<br/><span className="text-blue-600 font-serif italic font-normal">really sad...</span></>
              )}
            </h1>

            {/* Clean stacked dual explanation */}
            <div className="space-y-4 mb-8">
              <p className="text-base md:text-lg text-stone-600 leading-relaxed font-medium">
                {lang === "id" 
                  ? "Huby sedang sedih karena terjadi kendala koneksi pada sistem Hubify Social. Tim kami sedang aktif memperbaikinya, mohon tunggu sebentar ya!"
                  : "Huby is sad because of a system connection issue in Hubify Social. Our team is actively working to resolve it, please check back shortly!"}
              </p>
            </div>

            <div className="w-full mb-8">
              <MaintenanceCountdown lang={lang} />
            </div>

            {/* Modern creative action layout */}
            <div className="w-full flex flex-col sm:flex-row gap-4 mb-8">
              <button 
                onClick={() => window.location.reload()}
                className="bg-[#3B82F6] hover:bg-blue-600 text-white py-4 px-8 rounded-2xl text-sm font-bold tracking-wide transition-all duration-200 active:scale-[0.98] cursor-pointer shadow-[0_4px_16px_rgba(59,130,246,0.18)] hover:shadow-[0_6px_20px_rgba(59,130,246,0.28)]"
              >
                {lang === "id" ? "Coba Muat Ulang" : "Try Refreshing"}
              </button>
            </div>
...
function QuickAddEventModal({ workspace, onClose, onSaveSettings, initialEvent }: any) {
  const { lang } = useI18n();
...
function Dashboard({ user, profile, planDetails, onUpdateProfile, currentTheme, systemConfig }: any) {
  const { lang } = useI18n();
  ...
  const handleCreateWorkspace = async (name: string, copyFromId: string | null = null) => {
    ...
    if (ownedWorkspaces.length >= maxWorkspaces && profile?.plan !== "vip") {
  ...
  useEffect(() => {
    if (!user) return;
    setWsLoading(true);
    const q = query(collectionGroup(db, "members"), where("userId", "==", user.uid), limit(50));
      ...
      try {
        ...
        wsIds.forEach(id => {
          const u = onSnapshot(doc(db, "workspaces", id), (wsSnap) => {
            if (wsSnap.exists()) {
              results[id] = { ...wsSnap.data(), id: wsSnap.id, userRole: userRoles[id] || "viewer" };
            } else {
              delete results[id];
            }
            ...
            if (pendingIds.size === 0) {
              ...
              const currentWs = workspaceRef.current;
              if (listArr.length > 0) {
                const updatedCurrentWs = listArr.find(w => w.id === currentWs?.id);
                if (!currentWs || !updatedCurrentWs) {
                  setWorkspace(listArr[0]);
                } else {
                  setWorkspace(updatedCurrentWs);
                }
              }
  ...
  // Fetch content per month and cache it
  useEffect(() => {
    ...
    const fetchContent = async () => {
      ...
      const q = query(contentRef, where("year", "==", year), where("month", "==", month), limit(200));
      
      try {
        const snap = await getDocs(q);
        const docs = snap.docs.map(d => ({ ...d.data(), id: d.id }));
        contentCache.current[cacheKey] = docs;
        setContent(docs);
      } catch (e) {
  ...
  const handleSave = async (data: any, closeModal = true) => {
    ...
    const prevData = (!isNew && content) ? (content.find(c => c.id === itemId) || modal?.data) : null;
    if (prevData) {
    ...
    const targetWsId = data.workspaceId || workspace.id;

    if (cleanData.sharedUsers && Array.isArray(cleanData.sharedUsers)) {
      cleanData.sharedUids = cleanData.sharedUsers.map((u: any) => u.uid).filter(Boolean);
      cleanData.sharedEmails = cleanData.sharedUsers.map((u: any) => u.email?.toLowerCase()).filter(Boolean);
      cleanData.editorUids = cleanData.sharedUsers.filter((u: any) => u.role === "editor").map((u: any) => u.uid).filter(Boolean);
      cleanData.commenterUids = cleanData.sharedUsers.filter((u: any) => u.role === "commenter" || u.role === "editor").map((u: any) => u.uid).filter(Boolean);
    }
    ...
    const itemData = { 
      ...
      userId: cleanData.userId || user?.uid || "" 
    };
    console.log("Cleaned itemData:", itemData);
    
    try {
      await setDoc(doc(db, "workspaces", targetWsId, "content", itemId), itemData, { merge: true });
      console.log("Save successful!");
      
      // Update local state properly
      const oldItem = content.find(c => c.id === itemId);
      let newContent = [...content];
      
      // Remove from old month if changed
      if (oldItem && (oldItem.year !== itemData.year || oldItem.month !== itemData.month)) {
        newContent = newContent.filter(c => c.id !== itemId);
        setContentCache(prev => {
           const oldKey = `${oldItem.year}-${oldItem.month}`;
           if (prev[oldKey]) {
             return { ...prev, [oldKey]: prev[oldKey].filter(c => c.id !== itemId) };
           }
           return prev;
        });
      }
      
      // Add or update in new/current month
      const newKey = `${itemData.year}-${itemData.month}`;
      if (itemData.year === year && itemData.month === month) {
         if (oldItem && oldItem.year === itemData.year && oldItem.month === itemData.month) {
            newContent = newContent.map(c => c.id === itemId ? itemData : c);
         } else {
            newContent = [...newContent, itemData];
         }
         setContent(newContent);
      }
      setContentCache(prev => {
         if (prev[newKey]) {
            const exists = prev[newKey].find(c => c.id === itemId);
            return { 
               ...prev, 
               [newKey]: exists ? prev[newKey].map(c => c.id === itemId ? itemData : c) : [...prev[newKey], itemData] 
            };
         }
         return prev;
      });
  ...
  const deleteItem = async (id:string, force:boolean = false, targetWorkspaceId?: string) => { 
    const wsId = targetWorkspaceId || workspace?.id;
    if(!wsId || !id || isRestricted) return;
    
    const doDelete = async () => {
        try {
          const docRef = doc(db, "workspaces", wsId, "content", id);
          await deleteDoc(docRef); 
          setContent(prev => prev.filter(c => c.id !== id));
          setContentCache(prev => {
             const newCache = { ...prev };
             Object.keys(newCache).forEach(k => {
               newCache[k] = newCache[k].filter(c => c.id !== id);
             });
             return newCache;
          });
          setModal(null); 
  ...
  const archiveItem = async (id:string) => {
    ...
    try {
      ...
      setContentCache(prev => {
         const newCache = { ...prev };
         Object.keys(newCache).forEach(k => {
           newCache[k] = newCache[k].map(c => c.id === id ? { ...c, archived: true } : c);
         });
         return newCache;
      });
      setModal(null);
  ...
  const unarchiveItem = async (id:string) => {
    ...
    try {
      ...
      setContentCache(prev => {
         const newCache = { ...prev };
         Object.keys(newCache).forEach(k => {
           newCache[k] = newCache[k].map(c => c.id === id ? { ...c, archived: false } : c);
         });
         return newCache;
      });
      setModal(null);
    ...
    <div style={{display:"flex", height:"100vh", overflow:"hidden", background:"var(--theme-sidebar)"}}>
      ...
      {isUnverified && (
        <div style={{background:"#FBF5E3", borderBottom:"1px solid rgba(166,124,28,0.1)", padding:"12px 24px", display:"flex", alignItems:"center", justifyContent:"center", gap:12, zIndex:50}}>
          ...
          <button onClick={() => window.location.href="/profile"} style={{background:"#A67C12", color:"white", border:"none", padding:"6px 14px", borderRadius:20, fontSize:12, fontWeight:800, cursor:"pointer"}}>{lang === "id" ? "Lengkapi Sekarang" : "Complete Now"}</button>
        </div>
      )}
      ...
      {tab === "content_planner" && (
        <NavBar 
          ...
          onOpenAdd={()=>openAdd(1)} onOpenAddEvent={() => { setEditingEvent(null); setShowEventModal(true); }} isRestricted={isRestricted}
      ...
      <div style={{padding: (tab.startsWith("social") || ["soc_hub", "admin"].includes(tab)) ? "0" : "20px 24px 56px", position: "relative", minHeight: 0, flex: (tab.startsWith("social") || ["soc_hub", "admin"].includes(tab)) ? 1 : "none", display: "flex", flexDirection: "column"}}>
        {isRestricted && (
          <div style={{background:"#F8EAF0",border:"1px solid #9C2B4E",color:"#9C2B4E",padding:"12px 24px",borderRadius:12,marginBottom:24,display:"flex",alignItems:"center",gap:12,fontWeight:600}}>
            ...
            <button onClick={()=>window.location.href="/billing"} style={{background:"#9C2B4E",color:"#fff",padding:"8px 16px",borderRadius:8,fontSize:14,border:"none",cursor:"pointer"}}>{lang === "id" ? "Berlangganan Untuk Selengkapnya" : "Subscribe for More"}</button>
          </div>
        )}
      ...
      <AnimatePresence>
        {shareModal && <ShareWorkspaceModal key="share" workspace={workspace} userProfile={profile} planDetails={planDetails} onClose={()=>setShareModal(false)} />}
        </AnimatePresence>
      <AnimatePresence>
      ...
      <AnimatePresence>
        {modal && <ContentModal key="content" modal={modal} workspace={workspace} userProfile={profile} planDetails={planDetails} onSave={handleSave} onClose={()=>setModal(null)} onArchive={archiveItem} onRestore={unarchiveItem} onDelete={deleteItem} onDuplicate={(data:any) => {
...
function PublicView() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const contentSnap = await getDocs(collection(db, "workspaces", wsId, "content"));
      setContent(contentSnap.docs.map(d => d.data()));
    } catch (e) {
      console.error("PublicView fetch error:", e);
    }
    setLoading(false);
  };
       ...
       <div style={{maxWidth:1200, margin:"0 auto"}}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 32 }}>
            <div>
              ...
              <p style={{fontSize:14, color:"rgba(44,32,22,0.5)", margin: 0}}>Public Read-Only View • Terakhir diperbarui: {new Date().toLocaleDateString("id-ID")}</p>
            </div>
            <button 
              onClick={fetchData} 
              disabled={loading}
              style={{
                ...
                background: "white", border: "1px solid rgba(0,0,0,0.1)", color: "#2C2016",
                fontWeight: 600, cursor: loading ? "not-allowed" : "pointer", boxShadow: "0 2px 8px rgba(0,0,0,0.05)"
              }}
            >
