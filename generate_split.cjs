...
let logicStartIdx = code.indexOf('{', exportContentModalIdx) + 1;
...
const propsList = ["modal", "workspace", "userProfile", "planDetails", "onSave","onClose","onArchive","onRestore","onDelete","onDuplicate","pillars","platforms","contentTypes","pics","statuses","onSettingUpdate", "lang", "checkCanAddSocialAccount", "aiTokenLimit"];
...
// 2. ContentModal/useContentModalLogic.ts
const hookContent = `
...
export function useContentModalLogic(props: any) {
  ...
  const { lang } = useI18n();
  const { checkCanAddSocialAccount, aiTokenLimit } = usePlanLimits(planDetails);
...
fs.writeFileSync('src/ContentModal/useContentModalLogic.ts', hookContent);
...
console.log("Splitting generated.");
