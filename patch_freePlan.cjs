...
const freeLimitsReplacement = `  const dbFreePlan = plans.find(p => p.id === (isAnnual ? 'free-annual' : 'free-monthly'));
...
code = code.replace(/  const dbFreePlan = plans\.find(.|\n)*?const freeCaps = dbFreePlan\?\.capabilities \|\| \{.*?autopublishing: false.*?\};/i, freeLimitsReplacement);
