...
with open("src/PricingPage.tsx", "r") as file:
    content = file.read()

# 1. Add isAnnual state
content = content.replace(
    "  const [lang, setLang] = useState<'id' | 'en'>(() => {",
    "  const [isAnnual, setIsAnnual] = useState(false);\n  const [lang, setLang] = useState<'id' | 'en'>(() => {"
)

# 2. Replace Pricing Section Content
# The pricing section starts at `      {/* Pricing Section */}` and ends right before `          {/* FAQ Section */}`.
# I will use a regex to replace this whole block.

pricing_block = """      {/* Pricing Section */}
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 items-stretch mb-24">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-md border border-blue-100 hover:shadow-xl transition-shadow flex flex-col h-full relative">
              ...
              <button onClick={() => navigate('/login', { state: { mode: 'signup' }})} className="w-full py-3 rounded-xl font-bold bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors mt-auto">
            ...
            <div className="bg-[#0B2A4A] rounded-3xl p-6 shadow-2xl border border-blue-900 flex flex-col relative h-full transform lg:-translate-y-4">
              ...
              <button onClick={() => navigate('/login', { state: { mode: 'signup' }})} className="w-full py-3 rounded-xl font-bold bg-white text-[#0B2A4A] hover:bg-slate-100 transition-colors mt-auto shadow-lg">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-md border border-slate-200 hover:shadow-xl transition-shadow flex flex-col h-full">
              ...
              <button onClick={() => navigate('/login', { state: { mode: 'signup' }})} className="w-full py-3 rounded-xl font-bold bg-slate-900 text-white hover:bg-slate-800 transition-colors mt-auto">
...
end_idx = content.find("          {/* FAQ Section */}")

if start_idx != -1 and end_idx != -1:
    new_content = content[:start_idx] + pricing_block + "\n" + content[end_idx:]
    with open("src/PricingPage.tsx", "w") as file:
        file.write(new_content)
    print("Replaced pricing section successfully.")
else:
